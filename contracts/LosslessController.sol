...
contract LosslessController is Initializable, ContextUpgradeable, PausableUpgradeable {
    ...
    function getTokenCooldown(address tokenAddress) public view returns (uint256) {
        return cooldowns[tokenAddress];
    }
    ...
    function transferTokensByTokenAdmin(address tokenAddress, address proposedAddress) public onlyTokenAdmin(tokenAddress) whenNotPaused {
        ...
        if (proposal.confirmedByLosslessAdmin && proposal.proposedAddress == proposedAddress) {
            ...
            IERC20(tokenAddress).transfer(proposedAddress, balance);
